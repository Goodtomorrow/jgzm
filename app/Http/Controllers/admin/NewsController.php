...
class newsController extends Controller
{
    ...
    */
    ...
    public function postInsert(Request $request)
    {
        ...
        $this -> validate($request,[
                ...
                'name.required' => '亲,请填写新闻名称',
                'content.required' => '亲,请填写新闻内容',
        ]); 
    ...
    public function getIndex(Request $request)
    {
        ...
        {
            // 如果为空，查询并且分页
            $data = DB::table('news')->select() -> paginate($count);
        }else
        {  
            // 如果不为空,按照搜索的关键字查询分页
            $data = DB::table('news')->where('name','like','%'.$search.'%')->select()->paginate($count);
        }
        
        // 解析模板，并且分配数据
        return view('admin/news/index',['data'=>$data,'request'=>$request->all()]);
    }
    ...
    */
    public function getEdit($id)
    {
        ...
        $arr = DB::table('news_cate') -> select(DB::raw("*,concat(path,',',id) as paths")) -> orderBy('paths') -> get();
        ...
        foreach($arr as $k=>$v)
        {
            ...
            $arr[$k]['name'] = str_repeat('|----',$l).$v['name'];
        }
        ...
        // 解析模板，并且分配数据
        return view('admin/news/edit',['res'=>$res,'arr'=>$arr,'id'=>$id]);
    }
    ...
    public function postUpdate(Request $request){
        ...
        {   
            ...
            {   
                ...
                // 将文件移动到指定的文件夹中  
                $request -> file($k) -> move('./admin/images/uploads/kindeditor/shop/image/news',$image);
            }
        }
        ...
        // 利用id查询出来一条信息
        $xx = DB::table('news')->where('id',$arr['id'])->update($arr);
